...
> 当前项目以本地运行和求职展示为主，尚未进行真实线上部署。仓库已保留有价值的需求分析、用例规约、通信图、顺序图、类图、关系模型和数据库设计资料，便于面试时说明从需求分析到实现落地的完整过程。
...
- 设计资料完整：保留用例规约、通信图、顺序图、类图、数据库设计和开发复盘，适合面试讲解。
